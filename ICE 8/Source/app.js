...
angular.module('myApp', [])
    .controller('View1Ctrl', function ($scope, $http) {
        ...
        $scope.getVenues = function () {
            ...
            if (placeEntered != "") {
                $http({url: "http://127.0.0.1:8081/api/?apiVal=" + placeEntered, method: 'POST'}).then(function (data, status) {
                    console.log(data);
                    $scope.desc = data.data.DescriptionShort;
                    $scope.treatmentdesc = data.data.TreatmentDescription;
                });
            }
        }
